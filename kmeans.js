/*
    ...
    Code modified from Nathan Epstein's: 
...
module.exports = {
  ...
  clusters: function() {
    var pointsAndCentroidsSteps = kmeans(this.data(), {k: this.k(), iterations: this.iterations() });
    var pointsSteps = pointsAndCentroidsSteps.points_steps;
    var centroidsSteps = pointsAndCentroidsSteps.centroids_steps;
    var res = [];
    for(var i = 0; i < centroidsSteps.length; i++){
        var points = pointsSteps[i];
        var centroids = centroidsSteps[i];

        var mappedCentroids = centroids.map(function(centroid) { return {
            centroid: centroid.location(),
            points: points.filter(function(point) { return point.label() == centroid.label() }).map(function(point) { return point.location() }),
        };});
        res.push(mappedCentroids);
    }
    return res;
  },
...
function kmeans(data, config) {
  ...
  for (var iter = 0; iter < iterations; iter++) {
    ...
    //store previous points to check for convergence
    var converged = true;
    var prev_centroid_locations = [];
    for(var i = 0; i < centroids.length; i++){ prev_centroid_locations.push(centroids[i].location()); }

    var copy_points = [];
    var copy_centroids = [];

    points.forEach(function(point) { 
        var copy_point = new Point(JSON.parse(JSON.stringify(point.location())));
        copy_point.updateLabel(centroids);
        copy_points.push(copy_point);
        point.updateLabel(centroids);
    });
    centroids.forEach(function(centroid) { 
        var copy_centroid = new Centroid(JSON.parse(JSON.stringify(centroid.location())), JSON.parse(JSON.stringify(centroid.label())));
        copy_centroid.updateLocation(points);
        copy_centroids.push(copy_centroid)
        centroid.updateLocation(points) 
    });

    points_steps.push(jQuery.extend(true, [], copy_points));
    centroids_steps.push(jQuery.extend(true, [], copy_centroids));

    //check for convergence
    for(var i = 0; i < centroids.length; i++){
        if(JSON.stringify(centroids[i].location()) != JSON.stringify(prev_centroid_locations[i])){ converged = false; break; }
    }
    if(converged){ break; }
  };
  // return points and centroids
  return {
    points_steps: points_steps,
    centroids_steps: centroids_steps
  };
